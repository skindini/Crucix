  ...
  async complete(systemPrompt, userMessage, opts = {}) {
    ...
    const text = await new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        child.kill('SIGKILL');
        // Tagged like fetch's AbortSignal.timeout() error so callers can tell a
        // timeout from a CLI failure without parsing the message.
        const e = new Error(`claude-cli: timed out after ${timeout}ms`);
        e.name = 'TimeoutError';
        e.code = 'ETIMEDOUT';
        reject(e);
      }, timeout);
